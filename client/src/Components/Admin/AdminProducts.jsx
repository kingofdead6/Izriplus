...
const S = {
  ...
  select: { padding: "13px 14px", background: "var(--surface-soft)", border: "1px solid var(--line)", borderRadius: 12, color: "var(--ink)", fontSize: 15, outline: "none", minWidth: 180 },
  grid: { display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 18 },
  card: { background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 20, overflow: "hidden", transition: "transform .2s,border-color .2s,box-shadow .2s" },
  ...
  delBtn: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "9px", background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.3)", color: "#b91c1c", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" },
  ...
  textarea: { padding: "14px 16px", background: "var(--surface-soft)", border: "1px solid var(--line)", borderRadius: 12, color: "var(--ink)", fontSize: 15, outline: "none", fontFamily: "inherit", width: "100%", minHeight: 80, resize: "vertical", boxSizing: "border-box" },
  checkRow: { display: "flex", alignItems: "center", gap: 10, color: "var(--ink)", fontSize: 14, fontFamily: "inherit" },
        ...
        <div style={S.grid}>
          {filtered.map((item, i) => (
            <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              style={S.card}
              onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.borderColor = "rgb(var(--secondary-rgb) / .35)"; e.currentTarget.style.boxShadow = "0 12px 40px rgba(43,33,24,.16)"; }}
              ...
              <div style={S.cardBody}>
                ...
                {item.slug && <p style={{ fontSize: 12, color: "var(--muted)", margin: "3px 0 0" }}>/{item.slug}</p>}
