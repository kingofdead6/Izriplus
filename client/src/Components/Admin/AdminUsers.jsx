        ...
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginBottom: 36 }}>
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 22px", background: "linear-gradient(135deg,var(--secondary),var(--primary))", color: "#fff", fontFamily: "'Inter'", fontWeight: 700, fontSize: 15, border: "none", borderRadius: 14, cursor: "pointer" }}
            >
            ...
            <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
              ...
              <input
                ...
                style={{ width: "100%", paddingLeft: 44, paddingRight: 16, paddingTop: 13, paddingBottom: 13, background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, color: "var(--ink)", fontSize: 14, outline: "none", boxSizing: "border-box" }}
              />
            </div>
            <select
              value={filterRole}
              onChange={e => setFilterRole(e.target.value)}
              style={{ padding: "13px 18px", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 14, color: "var(--muted)", fontSize: 14, outline: "none", cursor: "pointer" }}
            >
          ...
          {/* Grid */}
          {loading ? (
            <div style={{ textAlign: "center", padding: "80px 0", color: "var(--muted)", fontSize: 20 }}>Loading admins...</div>
          ) : filteredUsers.length === 0 ? (
            <div style={{ textAlign: "center", padding: "80px 0", color: "var(--muted)", fontSize: 22, fontWeight: 300 }}>No admins found</div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 20 }}>
              {filteredUsers.map(user => (
                <motion.div
                  ...
                  whileTap={{ scale: 0.98 }}
                  whileHover={{ y: -6 }}
                  style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 20, overflow: "hidden", backdropFilter: "blur(16px)" }}
                >
                  <div style={{ padding: 24, textAlign: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
                      <button
                        ...
                        style={{ flex: 1, padding: "10px", background: "var(--surface-soft)", border: "1px solid var(--line)", borderRadius: 12, color: "var(--muted)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600 }}
                      >
                        <Edit size={15} /> Edit
                      </button>
                      <button
                        onClick={() => { setSelectedUser(user); setShowPasswordModal(true); }}
                        style={{ flex: 1, padding: "10px", background: "rgba(251,146,60,.1)", border: "1px solid rgba(251,146,60,.25)", borderRadius: 12, color: "#c2410c", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600 }}
                      >
                    ...
                    {user.usertype !== "superadmin" && (
                      <button
                        ...
                        style={{ width: "100%", padding: "10px", marginTop: 10, background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.25)", borderRadius: 12, color: "#b91c1c", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600 }}
                      >
...
const btnPrimary = { flex: 1, padding: "14px", background: "linear-gradient(135deg,var(--secondary),var(--primary))", color: "#fff", fontFamily: "'Inter'", fontWeight: 700, fontSize: 15, border: "none", borderRadius: 14, cursor: "pointer" };
const btnSecondary = { flex: 1, padding: "14px", background: "transparent", border: "1px solid var(--line)", color: "var(--muted)", fontFamily: "'Inter'", fontWeight: 700, fontSize: 15, borderRadius: 14, cursor: "pointer" };
...
function Modal({ children, title, onClose }) {
  return (
    ...
    >
      <motion.div
        style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 24, width: "100%", maxWidth: 520, maxHeight: "85vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 80px rgba(43,33,24,.22)" }}
...
function FieldInput({ label, ...props }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 12, color: "var(--muted)", marginBottom: 8, fontFamily: "'Inter'", letterSpacing: ".04em" }}>{label.toUpperCase()}</label>
      <input style={{ width: "100%", padding: "13px 16px", background: "var(--surface-soft)", border: "1px solid var(--line)", borderRadius: 12, color: "var(--ink)", fontSize: 15, outline: "none", boxSizing: "border-box" }} {...props} />
    </div>
  );
}
...
function FieldSelect({ label, children, ...props }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 12, color: "var(--muted)", marginBottom: 8, fontFamily: "'Inter'", letterSpacing: ".04em" }}>{label.toUpperCase()}</label>
      <select style={{ width: "100%", padding: "13px 16px", background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 12, color: "var(--ink)", fontSize: 15, outline: "none", boxSizing: "border-box" }} {...props}>
        {children}
      </select>
    </div>
  );
